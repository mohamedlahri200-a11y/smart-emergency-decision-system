...
// src/components/dashboard/biologiste/BiologisteAideIAPage.tsx
...
import {
    AutoAwesomeRounded, SearchRounded, ScienceRounded, ArrowBackRounded,
    MedicalInformationRounded, WarningAmberRounded, CheckCircleRounded,
    PriorityHighRounded,
...
const BiologisteAideIAPage: React.FC = () => {
    ...
    const ouvrirConversation = (e: ExamenAvecPriorite) => {
        ...
        const historique: MessageChat[] = [
            {
                id: "intro",
                auteur: "assistant",
                texte: `Analyse biologique pour ${e.patientNomComplet} — ${e.typeExamen}, demandée par ${e.medecinPrescripteurNomComplet}.`,
            },
            {
                id: "urgence",
                auteur: "assistant",
                texte: prio?.label === "Rouge — Vital"
                    ? "⚠️ Ce patient est classé priorité vitale par le triage — à traiter en tout premier."
                    : "Classement par niveau d'urgence : ce patient occupe sa place dans la file selon la priorité déterminée au triage.",
                carte: (
                    <Stack direction="row" gap={1} flexWrap="wrap">
                        {prio && (
                            <Chip
                                size="small"
                                icon={prio.label.startsWith("Rouge") ? <PriorityHighRounded sx={{ fontSize: "14px !important" }} /> : undefined}
                                label={prio.label}
        ...
        if (e.description) {
            historique.push({ id: "motif", auteur: "assistant", texte: `Motif clinique transmis : "${e.description}"` });
        }
        ...
        if (e.statutExamen === "TERMINE" && e.resultat) {
            historique.push({
                id: "resultat",
                auteur: "assistant",
                texte: "Compte rendu biologique déjà finalisé et transmis au médecin urgentiste :",
                carte: (
            ...
            if (e.compteRenduCritique) {
                historique.push({
                    id: "critique",
                    auteur: "assistant",
                    texte: `Anomalies critiques détectées automatiquement : ${e.motsClesCritiquesDetectes.join(", ")}. Le médecin urgentiste a été alerté immédiatement.`,
                });
            }
        } else {
            historique.push({
                id: "invite",
                auteur: "assistant",
                texte: "Ouvrez la fiche complète pour saisir les valeurs et vos observations. L'assistant : structure automatiquement le compte rendu biologique, détecte les anomalies critiques (CRP, procalcitonine, leucocytes, troponine, lactate...), et déclenche une alerte immédiate vers le médecin urgentiste si nécessaire. Vous gardez la main pour valider ou modifier avant envoi.",
            });
        }
    ...
    const handleUpdated = async () => {
        ...
        setMessages((prev) => [
            ...prev,
            {
                id: `maj-${Date.now()}`,
                auteur: "assistant",
                texte: "Compte rendu enregistré et transmis au médecin urgentiste. Il apparaîtra désormais dans sa fiche patient, avec alerte automatique si un résultat critique a été détecté.",
            },
        ]);
    };
    ...
    return (
        <DashboardLayout
            title="Aide IA — Médecin Biologiste"
            subtitle="Assistant conversationnel pour l'interprétation, la détection d'anomalies et la rédaction du compte rendu biologique"
        >
                        ...
                        <Box sx={{ overflowY: "auto", flex: 1 }}>
                            ...
                            {!loadingListe && filtres.map((e) => {
                                ...
                                return (
                                    ...
                                    >
                                        <Avatar sx={{ width: 34, height: 34, bgcolor: "#EEF1F6", color: "#0D47A1" }}><ScienceRounded fontSize="small" /></Avatar>
                    ...
                    <Grid size={{ xs: 12, md: 8.5 }} sx={{ height: "100%" }}>
                        <Paper sx={{ height: "100%", borderRadius: 4, display: "flex", flexDirection: "column", overflow: "hidden" }}>
                            <Stack direction="row" alignItems="center" gap={1.5} sx={{ p: 2, borderBottom: "1px solid #EEF1F6" }}>
                                ...
                                <Box sx={{ flex: 1 }}>
                                    ...
                                    <Typography variant="caption" color="text.secondary">
                                        Interprétation assistée, détection d'anomalies, génération du compte rendu
                                    </Typography>
                                </Box>
                            </Stack>
                            ...
                            <Stack direction="row" gap={1.25} flexWrap="wrap" alignItems="center" sx={{ px: 2.5, py: 2, backgroundColor: "#fff" }}>
                                ...
                                >
                                    {examenActif.statutExamen === "TERMINE" ? "Revoir la fiche complète" : "Ouvrir la fiche — saisir le compte rendu"}
                                </Button>
                                {examenActif.compteRenduCritique && (
                                    <Chip size="small" icon={<WarningAmberRounded sx={{ fontSize: "14px !important" }} />} color="error" label="Résultat critique détecté" />
                                )}
                            </Stack>
                        </Paper>
                    </Grid>
...
export default BiologisteAideIAPage;