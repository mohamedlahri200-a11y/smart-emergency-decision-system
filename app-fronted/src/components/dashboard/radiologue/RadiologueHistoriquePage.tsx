// src/components/dashboard/radiologue/RadiologueHistoriquePage.tsx
...
const columns: GridColDef<ExamenResponseDTO>[] = [
    ...
    { field: "medecinPrescripteurNomComplet", headerName: "Prescripteur", width: 180 },
    {
        ...
        renderCell: (params) => (
            <Chip
                size="small"
                label={params.value === "TERMINE" ? "Terminé" : "Annulé"}
...
const RadiologueHistoriquePage: React.FC = () => {
    ...
    const fetchHistorique = useCallback(async (): Promise<void> => {
        setLoading(true);
        try {
            const data = await examenService.getAll("IMAGERIE");
            const traites = data
    ...
    return (
        <DashboardLayout title="Historique" subtitle="Examens d'imagerie déjà traités">
...
export default RadiologueHistoriquePage;
